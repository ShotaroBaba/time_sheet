...
cp default_root.txt .mysql_secrets
cp default_admin.txt .mysql_secrets
...
openssl rand -base64 32 | openssl rsautl -encrypt -pubin -inkey $pub_key -out $encrypted_pass

pepper=$(openssl rand -base64 32);

#pass_str=$(docker exec mysql_server bash -c "mysql --defaults-extra-file=/.mysql_secrets/default_root.txt")
...
BY '"$(openssl rsautl -decrypt -inkey $priv_key -in $encrypted_pass)"';\""

# Generate admin password.
openssl rand -base64 32 | openssl rsautl -encrypt -pubin -inkey $pub_key -out $encrypted_admin_pass
...
BY '"$(openssl rsautl -decrypt -inkey $priv_key -in $encrypted_admin_pass)"';\""

# Create password for a table manager.
# openssl rand -hex 48 | openssl rsautl -encrypt -pubin -inkey $pub_key -out $encrypted_table_client_pass

# This script is for testing purpose.
openssl rand -base64 32 | openssl rsautl -encrypt -pubin -inkey $pub_key -out $encrypted_table_client_pass

openssl rand -base64 32 | openssl rsautl -encrypt -pubin -inkey $pub_key -out $encrypted_table_admin_pass
...
find .mysql_secrets -name '*.pem' -o -name '*.enc' | while read p; do chmod 400 $p; done
find .mysql_secrets -name '*.txt' | while read p; do rm $p; done
...
docker exec mysql_server \
bash -c "mysql -uroot -hmysql_server -p"$(decrypt_script/root_decrypt.sh)" \
...
docker exec mysql_server \
bash -c "mysql -uroot -hmysql_server -p"$(decrypt_script/root_decrypt.sh)" \
 -e \"CREATE USER IF NOT EXISTS 'time_sheet_client' IDENTIFIED WITH mysql_native_password BY '"$(openssl rsautl -decrypt -inkey $priv_key -in $encrypted_table_client_pass)"'\""

# Create a admin for `time sheet` database.
docker exec mysql_server \
bash -c "mysql -uroot -hmysql_server -p"$(decrypt_script/root_decrypt.sh)" \
 -e \"CREATE USER IF NOT EXISTS 'time_sheet_admin' IDENTIFIED WITH mysql_native_password BY '"$(openssl rsautl -decrypt -inkey $priv_key -in $encrypted_table_admin_pass)"'\""

# Replace sample with generated passwords. 
cat www/.secret/.config_sample.php | \
sed "s|_____time_sheet_pass_____|"$(openssl rsautl -decrypt -inkey $priv_key -in $encrypted_table_client_pass)"|g" | \
sed "s|_____pepper_string_____|"$pepper"|" > www/.secret/.config.php

docker exec mysql_server \
bash -c "mysql -uroot -hmysql_server -p"$(decrypt_script/root_decrypt.sh)" < /create_database.sql" 